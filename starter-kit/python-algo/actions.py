...
""" Checks if the specified unit can spawn in the specified locations """
def units_deployed_locations(game_state, unit_type, locations):
    deployed = []
    for location in locations:
        ...
        for unit in game_state.game_map[location[0], location[1]]:
            if unit.stationary:
                existing_unit = unit
        if existing_unit:
            deployed.append(existing_unit.unit_type == unit_type)
        else:
            deployed.append(False)
    return deployed

""" Checks if the specified units are upgraded in the specified locations """
def units_upgraded_locations(game_state, unit_type, locations):
    upgraded = []
    for location in locations:
        existing_unit = None
        for unit in game_state.game_map[location[0], location[1]]:
            if unit.stationary:
                existing_unit = unit
        if existing_unit:
            upgraded.append(existing_unit.upgraded)
        else:
            upgraded.append(False)
    return upgraded

""" Checks if the specified units need repair in the specified locations """
def units_repair_locations(game_state, unit_type, locations, critical_health=0.3):
    need_repair = []
    for location in locations:
        existing_unit = None
        for unit in game_state.game_map[location[0], location[1]]:
            if unit.stationary:
                existing_unit = unit
        if existing_unit:
            need_repair.append((float(existing_unit.health) / float(existing_unit.max_health)) < critical_health)
        else:
            need_repair.append(False)
    return need_repair
